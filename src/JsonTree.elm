...
parseValue json =
    ...
    in
    Decode.decodeValue decoder json |> Result.mapError Decode.errorToString
...
viewScalar : List (Attribute msg) -> String -> Node -> Config msg -> List (Html msg)
...
viewDict depth dict keyPath config state =
    let
        ...
        viewListItem ( fieldName, node ) =
            li
                css.li
                ([ span css.fieldName [ text fieldName ]
                 , text ": "
                 ]
